...
class MainGUI(Frame):
    ...
    def make_button(self, row, col, value):
        ...
        button.grid(row=row, column=col, sticky=NSEW, columnspan=col_span)
    ...
    def handle_button_press(self, button_value):
        ...
        if evaluate:
            try:
                ...
                if len(result) > 14:
                    result = str(int(result[:14])/(10**13)) + f"e+{len(result)-1}"
        ...
        if len(self.display["text"]) > 13:
            self.display["text"] = "Error: Too many chars"
            self.errored = True
            return

        if self.errored or self.calculated:
            self.display["text"] = ""
            self.errored = False
            self.calculated = False
